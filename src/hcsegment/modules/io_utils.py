...
def get_positions(file_list: List[str]) -> Tuple[Set[str], Set[str], Set[str]]:
...
def get_files_in_path(inp_path: str) -> List[str]:
    """
    Given the input path, returns the images in the path
    """
...
def write_image(data: np.ndarray, output: str, well: str) -> None:
    """
    Write image data in tiff format
